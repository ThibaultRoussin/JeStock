...
/**
 * Created by Thibault on 07/06/2018.
 */

public class ListRenduFragment extends ListFragment {

    public interface OnMaterielSelectedListener {
        void onMaterielSelected(int id);
    }

    private ListRenduFragment.OnMaterielSelectedListener listener;
    public static List<Emprunts> empruntList2 = new ArrayList<>();

    @Override
    public void onAttach(Context context) {
        super.onAttach(context);
        if(getActivity()instanceof ListRenduFragment.OnMaterielSelectedListener) {
            listener = (ListRenduFragment.OnMaterielSelectedListener) getActivity();
        }
    }
    ...
    private void chargerListe() {
        ...
        call.enqueue(new Callback<List<Emprunts>>() {
            ...
            public void onResponse(Call<List<Emprunts>> call, Response<List<Emprunts>> response) {
                ...
                for (Emprunts emprunt : empruntList){

                    if (emprunt.getReference() == ReferenceEmprunt.getReference()) empruntList2.add(emprunt);
                }

                if (empruntList2.size() != 0){
                    ListAdapterEmprunts adapter = new ListAdapterEmprunts(getActivity(), empruntList2);
                    setListAdapter(adapter);
                }
                else{
                    ListAdapterEmprunts adapter = new ListAdapterEmprunts(getActivity(), new ArrayList<Emprunts>());
                    setListAdapter(adapter);
                }
            ...
            public void onFailure(Call<List<Emprunts>> call, Throwable t) {
                ...
                Toast.makeText(getContext(), "Pas de rendus matériels possibles", Toast.LENGTH_SHORT).show();
            }
        });
    }
